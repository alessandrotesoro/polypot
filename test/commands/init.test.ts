...
describe('polypot init', () => {
  ...
  it('--cwd echoes the custom path in the stub output', async () => {
    ...
    const {stdout, error} = await runCommand(['init', '--cwd', customCwd])
    expect(error).to.equal(undefined)
  ...
  it('does not create or modify any file (Phase 1 stub regression guard)', async () => {
    ...
    try {
      ...
      expect(error).to.equal(undefined)
      expect(fs.existsSync(path.join(tempDir, '.polypot'))).to.equal(false)
      expect(fs.readFileSync(gitignorePath, 'utf8')).to.equal(originalGitignore)
    } finally {
