...
export type OpenAIConnectionResult =
  | {readonly ok: true}
  | {readonly ok: false; readonly reason: 'missing-key' | 'auth-failed' | 'network-error' | 'unknown-error'; readonly message: string}
...
const defaultClientFactory: OpenAIClientFactory = (apiKey) => new OpenAI({
  apiKey,
  maxRetries: 0,
