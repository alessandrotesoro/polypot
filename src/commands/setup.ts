...
import {buildSetupConfig, collectSetupAnswers, confirmSetupUpdate} from '../setup/prompts.js'
...
`
  ...
  static override flags = {
    ...
    show: Flags.boolean({
      summary: 'Print the resolved global config paths, non-secret config, and secret presence.',
      env: polypotEnv('show'),
    }),
  ...
  public async run(): Promise<void> {
    ...
    const storeOptions = {configDir: this.config.configDir, cwd: process.cwd()}
    const paths = resolveConfigPaths(storeOptions)

    if (flags.show) {
      const [{config, exists: hasGlobalConfig}, secrets] = await Promise.all([
      ...
      this.log(`OPENAI_API_KEY: ${secrets.hasOpenaiApiKey ? 'present' : 'missing'}`)
      if (hasGlobalConfig) {
    ...
    if (flags['non-interactive']) {
      this.error('polypot setup is interactive. Remove --non-interactive or edit the global config files directly.', {
        exit: 1,
      })
    }
    ...
    if (hasExistingFiles && !flags.force && !(await confirmSetupUpdate())) {
      this.log('Setup cancelled. Existing global config was not changed.')
      return
    }
    ...
    const answers = await collectSetupAnswers(existingConfig, existingSecrets)

    if (answers.validateConnection && answers.openaiApiKey !== undefined) {
      const validation = await validateOpenAIConnection(answers.openaiApiKey)
    ...
    this.log(`global config: ${paths.globalYaml}`)
    this.log(`global secrets: ${paths.globalEnv}`)
    this.log(`OPENAI_API_KEY: ${answers.openaiApiKey === undefined && !existingSecrets.hasOpenaiApiKey ? 'missing' : 'present'}`)
  }

  private async readExistingConfig(force: boolean, storeOptions: ResolveConfigPathsOptions): Promise<PolypotConfig> {
    try {
      return await readGlobalConfig(storeOptions)
    } catch (error) {
      if (!force) throw error
      this.warn(`${error instanceof Error ? error.message : String(error)}. Continuing with defaults because --force was provided.`)
